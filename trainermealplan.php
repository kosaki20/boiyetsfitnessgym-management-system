...
$unread_notifications_count = getUnreadNotificationCount($conn, $current_trainer_id);

// Mark all as read if requested
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_all_read'])) {
    if (markAllNotificationsAsRead($conn, $current_trainer_id)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_as_read'])) {
    ...
    // Assuming markNotificationAsRead is in notification_functions.php or we use markAll as a template
    // For now, let's stick to centralized ones
    echo json_encode(['success' => true]); // Placeholder for specific AJAX if needed
    exit;
}

// Get notifications for display
$notifications = getTrainerNotifications($conn, $current_trainer_id);
